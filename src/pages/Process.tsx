...
const milestones = [
  { percent: "10%", label: "Booking Amount", desc: "Required to initiate registration and site assessment." },
  { percent: "80%", label: "Second Installment", desc: "Paid before the procurement and installation of components." },
  { percent: "10%", label: "Final Payment", desc: "Paid after construction and successful net metering." }
];

const ProcessPage = () => {
  return (
    <MainLayout>
      <section className="pt-32 pb-24 bg-gradient-to-b from-background to-card">
        <div className="container px-4">
          <div className="max-w-4xl mx-auto space-y-6 text-center mb-24">
            ...
            >
              <Smartphone size={16} />
              <span className="text-sm font-semibold tracking-wider uppercase">PM SURYA GHAR APP</span>
            </motion.div>
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              We've streamlined the solar installation process. From the first click to the final connection, we are with you at every step.
            </p>
          </div>
             ...
             <div className="space-y-12 sm:space-y-24">
                {steps.map((step, idx) => (
                   <motion.div
                      ...
                      className={`relative flex flex-col ${idx % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-8 md:gap-16`}
                   >
                      <div className="flex-1 text-center md:text-left">
                         <div className={`flex flex-col ${idx % 2 === 0 ? 'md:items-end' : 'md:items-start'} gap-4`}>
                            <span className="text-primary font-bold tracking-widest uppercase text-sm">{step.badge}</span>
                      ...
                      <div className="relative z-10 w-16 h-16 rounded-full bg-primary flex items-center justify-center text-primary-foreground shadow-xl shadow-primary/20 shrink-0">
                         {step.icon}
                      </div>
         ...
         <div className="container px-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                  ...
                  <div className="grid gap-6">
                     {milestones.map((milestone, idx) => (
                        <div key={idx} className="glass-card p-6 rounded-2xl flex items-center gap-6">
                           <div className="w-16 h-16 rounded-xl bg-background border flex items-center justify-center font-black text-xl text-primary">
                              {milestone.percent}
                           </div>
                           <div>
                              <h4 className="font-bold text-lg">{milestone.label}</h4>
               ...
               <motion.div
                  ...
                  viewport={{ once: true }}
                  className="bg-primary/5 border border-primary/20 rounded-3xl p-8 md:p-12 space-y-8"
               >
                  <div className="p-4 bg-primary/10 rounded-2xl inline-block mb-4">
                     <Zap className="text-primary" size={32} />
                  </div>
                  <h3 className="text-2xl font-bold">Important Note on Subsidy</h3>
                  <p className="text-muted-foreground leading-relaxed">
                     One of the common myths is about the subsidy timing. Please note that the <strong>Subsidy is credited after the commissioning report</strong> is filed. It is not immediately returned to the customer after installation.
                  </p>
